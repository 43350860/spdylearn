...
public class RecordingReceiver implements Response.Receiver {
	public static final long TIMEOUT_MILLIS = TimeUnit.SECONDS.toMillis(200000);//ԭʼ10s

	private final List<RecordedResponse> responses = new ArrayList<RecordedResponse>();

	@Override
	public synchronized void onFailure(Failure failure) {
		responses.add(new RecordedResponse(failure.request(), null, null,
				failure));
		notifyAll();
	}
	...
	public synchronized void onResponse(Response response) throws IOException {
		responses.add(new RecordedResponse(response.request(), response,
	 ...
	 */
	public synchronized RecordedResponse await(Request request)
			throws Exception {
		long timeoutMillis = TimeUnit.NANOSECONDS.toMillis(System.nanoTime())
				+ TIMEOUT_MILLIS;
		while (true) {
