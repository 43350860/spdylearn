...
public final class SpdyServer implements IncomingStreamHandler {
	...
	private void send404(SpdyStream stream, String path) throws IOException {
		List<String> responseHeaders = Arrays.asList(":status", "404",
				":version", "HTTP/1.1", "content-type", "text/plain");
	...
	private void serveDirectory(SpdyStream stream, String[] files)
			throws IOException {
		List<String> responseHeaders = Arrays.asList(":status", "200",
				":version", "HTTP/1.1", "content-type",
	...
	private void serveFile(SpdyStream stream, File file) throws IOException {
		...
		stream.reply(Arrays.asList(":status", "200", ":version", "HTTP/1.1",
				"content-type", contentType(file)), true);
		OutputStream out = stream.getOutputStream();
		int count;
